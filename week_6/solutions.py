...
def tree_sum(tree: Optional[BinaryTree]) -> int:
    ...
    if not tree:
        return 0
    return tree.value + tree_sum(tree.right) + tree_sum(tree.left)
...
def nested_max(nl: NestedList) -> int:
    ...
    8
    """
    if isinstance(nl, Leaf):
        return nl.value
    
    curr_max = None
    for sublist in nl.children:
        if not curr_max:
            curr_max = nested_max(sublist)
        else:
            curr_max = max(curr_max, nested_max(sublist))
    return curr_max
...
def is_valid_bst(tree: Optional[BinaryTree],
    ...
    prev = inorder_traversal[0] - 1
    for val in inorder_traversal:
        if prev > val:
            return False
        prev = val
...
def fs_count_name(node: FSNode, target: str) -> int:
    ...
    """
    if node.is_file():
        return int(node.name == target)
    
    count = 0
    for child in node.children:
        count += fs_count_name(child, target)
    return count
