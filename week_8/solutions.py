...
class BinaryNumber:
    ...
    def __rshift__(self, n: int) -> BinaryNumber:
        ...
        new_bits = []
        for i in range(len(self.bits) - n):
...
def add_binary(a: BinaryNumber, b: BinaryNumber) -> BinaryNumber:
    ...
    for i in range(max(len_a, len_b)):
        reversed_idx = (i * -1) - 1
        total_bits = int(bits_a[reversed_idx]) + int(bits_b[reversed_idx]) + int(carry) 
        result.append(total_bits % 2 == 1)
        carry = total_bits >= 2
...
def dict_depth(d: dict) -> int:
    ...
    for val in d.values():
        if isinstance(val, dict):
            val_depth = dict_depth(val)
            if val_depth > nested_depth:
                nested_depth = val_depth 
