...
def count_nodes(expr: Expr) -> int:
    ...
    if expr.left is not None:
        total_expr += count_nodes(expr.left)
    if expr.right is not None:
        total_expr += count_nodes(expr.right)
...
def tree_depth(expr: Expr) -> int:
    ...
    return 1 + max(tree_depth(expr.left), tree_depth(expr.right))
...
def substitute(expr: Expr, old_val: int, new_val: int) -> Expr:
    ...
    if isinstance(expr, Lit):
        if expr.value == old_val:
            new_expr = Lit(new_val)
        else:
            new_expr = Lit(expr.value)
        return new_expr
    
    new_left = substitute(expr.left, old_val, new_val)
    new_right = substitute(expr.right, old_val, new_val)
...
def list_max(node: "ListNode | None") -> "int | None":
    ...
    if node.next:
        return max(node.value, list_max(node.next))
    else:
        return node.value
...
def list_count(node: "ListNode | None", target: int) -> int:
    ...
    return list_count(node.next, target) + add_on
...
class Vector(LinearObject):
    ...
    def scale(self, factor: float) -> "Vector":
        new_elements = []
        for scalar in self.elements:
            new_elements.append(scalar * factor)
