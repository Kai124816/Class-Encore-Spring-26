...
class Director(Manager):
    ...
    def annual_salary(self) -> float:
        return super().annual_salary() + self.stock_grant
...
class ElectricGuitar(Instrument):
    ...
    def play(self) -> str:
        return f"{self.name} shreds at volume {self.amp_volume}."

    def turn_up(self, amount: int) -> None:
        self.amp_volume = min(10, self.amp_volume + amount)

    def turn_down(self, amount: int) -> None:
        self.amp_volume = max(0, self.amp_volume - amount)

    def __str__(self) -> str:
        return f"ElectricGuitar({self.name}, vol={self.amp_volume})"
...
class Playlist:
    ...
    def __str__(self) -> str:
        return f"Playlist({self.name}, {self.__len__()} songs)"
...
class Voter:
    ...
    def vote(self, candidate: str) -> None:
        if not self.voted:
            if candidate in Voter.votes.keys():
                Voter.votes[candidate] += 1
            else:
                Voter.votes[candidate] = 1
            self.voted = True
...
def highest_earner(employees: list) -> "Employee":
    ...
    for worker in employees:
        curr_salary = worker.annual_salary()
        if curr_salary > highest_salary:
            highest_earner = worker
